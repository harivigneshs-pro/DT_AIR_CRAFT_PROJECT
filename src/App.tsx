        ...
        <aside className="w-[320px] bg-slate-900/60 border-l border-slate-800 flex flex-col overflow-y-auto shadow-2xl z-10 custom-scrollbar">
          ...
          <div className="p-5">
            ...
            <button onClick={() => setShowPatterns(!showPatterns)} disabled={!searchData}
              className={`w-full flex items-center justify-center gap-2 py-3 rounded-lg text-xs font-bold uppercase tracking-wider transition-all border disabled:opacity-50 ${
                showPatterns ? 'bg-sky-500/20 border-sky-500/50 text-sky-400' : 'bg-slate-800 border-slate-700 text-slate-400'
              }`}
            >
              Draw Exp. Square 
            </button>
