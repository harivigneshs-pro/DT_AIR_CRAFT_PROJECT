...
import {
  Wind, Navigation, Plane, Activity, Crosshair, Terminal, Info, Settings2, RefreshCw, Bot, Route, Download, LogOut, MapPin, Radio, Clock, AlertCircle, Upload
} from 'lucide-react';
...
export default function App() {
  ...
  // States
  const [aircraftProfiles, setAircraftProfiles] = useState(DEFAULT_AIRCRAFT_PROFILES);
  const [aircraftId, setAircraftId] = useState('N7824K - Cessna 172');
  const [selectedProfile, setSelectedProfile] = useState(DEFAULT_AIRCRAFT_PROFILES[0]);
  ...
  const [newGlideRatio, setNewGlideRatio] = useState(10);
  ...
  const exportMissionData = () => {
    ...
    a.href = url;
    a.download = `sar-mission-${aircraftId.replace(/[^a-zA-Z0-9]/g, '-')}-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
    URL.revokeObjectURL(url);
    addLog('Mission data exported successfully.', 'success');
  };
  ...
  const handleFileLoad = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setAircraftId(data.aircraftId || aircraftId);
        setSelectedProfile(aircraftProfiles.find(p => p.id === data.selectedProfile?.id) || selectedProfile);
        setAltitude(data.altitude || altitude);
        setHeading(data.heading || heading);
        setWindSpeed(data.windSpeed || windSpeed);
        setWindDirection(data.windDirection || windDirection);
        setLkp(data.lkp || null);
        setSearchData(data.searchData || null);
        setActiveAssets(data.activeAssets || []);
        setLogs(data.logs || []);
        setMissionStartTime(data.missionStartTime || null);
        addLog('Mission data loaded successfully.', 'success');
      } catch (error) {
        addLog('Failed to load mission data.', 'error');
      }
    };
  ...
  const addCustomProfile = () => {
    if (!newProfileName.trim() || newGlideRatio <= 0) return;
    const newProfile = {
      id: `custom-${Date.now()}`,
      name: newProfileName.trim(),
      glideRatio: newGlideRatio
    };
    setAircraftProfiles(prev => [...prev, newProfile]);
    setNewProfileName('');
    setNewGlideRatio(10);
    addLog(`Custom aircraft profile "${newProfile.name}" added.`, 'success');
  };
      ...
      <header className={`h-16 border-b border-slate-800 backdrop-blur-md flex items-center justify-between px-6 z-50 shadow-md ${missionStartTime ? 'bg-red-950/20' : 'bg-slate-900/80'} transition-colors duration-1000`}>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3">
            <button onClick={exportMissionData} className="flex items-center gap-2 p-2 hover:bg-slate-800 rounded-lg transition-colors text-slate-400 hover:text-white" title="Export Briefing">
              <Download className="w-4 h-4" /> <span className="text-xs font-medium tracking-wide uppercase hidden lg:block">Export</span>
            </button>
            <button onClick={loadMissionData} className="flex items-center gap-2 p-2 hover:bg-slate-800 rounded-lg transition-colors text-slate-400 hover:text-white" title="Load Mission">
              <Upload className="w-4 h-4" /> <span className="text-xs font-medium tracking-wide uppercase hidden lg:block">Load</span>
            </button>
      ...
      <main className="flex flex-1 overflow-hidden relative">
        ...
        <aside className="w-[320px] border-r border-slate-800 bg-slate-900/40 p-5 flex flex-col gap-5 overflow-y-auto z-10 shadow-2xl custom-scrollbar">
          ...
          <section className="bg-slate-950/60 border border-slate-800 p-3 rounded-xl shadow-inner">
            ...
            <select value={selectedProfile.id} onChange={(e) => setSelectedProfile(aircraftProfiles.find(p => p.id === e.target.value) || aircraftProfiles[0])}
            ...
            <div className="space-y-2 mb-3">
              <div className="flex gap-1">
                <input type="text" placeholder="Profile Name" value={newProfileName} onChange={(e) => setNewProfileName(e.target.value)}
                  className="flex-1 bg-slate-900 border border-slate-700 rounded text-xs py-1 px-2 focus:outline-none focus:border-emerald-500 text-emerald-200" />
                <input type="number" placeholder="Glide Ratio" value={newGlideRatio} onChange={(e) => setNewGlideRatio(Number(e.target.value))}
                  className="w-16 bg-slate-900 border border-slate-700 rounded text-xs py-1 px-2 focus:outline-none focus:border-emerald-500 text-emerald-200" />
              </div>
              <button onClick={addCustomProfile} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white text-xs py-1 rounded transition-colors">
                Add Custom Profile
              </button>
            </div>
        ...
        {/* Real-World Map Area */}
        <div className="flex-1 relative bg-[#0f172a] z-0 overflow-hidden">
          {/* Tactical Grid Overlay */}
          <div className="absolute inset-0 z-[400] pointer-events-none opacity-30"
            style={{
